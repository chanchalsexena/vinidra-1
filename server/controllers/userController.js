...
export const registerUser = CatchAsyncError(async (req, res, next) => {
    ...
    try {
        ...
        if (username.length < 6) {
            throw new ErrorHandler('Username must be at least 6 characters long', 400);
        }
        ...
        if (!hasSpecialCharacter.test(username)) {
            throw new ErrorHandler('Username must contain at least one special character', 400);
        }
        if (!hasAlphabet.test(username)) {
            throw new ErrorHandler('Username must contain at least one alphabet', 400);
        }
        if (!fullname) {
...
export const loginUser = CatchAsyncError(async (req, res, next) => {
    ...
    try {
        ...
        if (!user.isVerified) {
            throw new ErrorHandler('Your account is not verified. Please verify your account', 400);
        }
...
export const changePassword = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        if (oldPassword === newPassword) {
            throw new ErrorHandler('Password is same as previous password. Please enter a new password', 400);
        }
        await user.save();
...
export const updateAvatar = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const myCloud = await cloudinary.v2.uploader.upload(fileUri.content, {
            folder: "vinidraexam/profilepic", crop: "scale"
        });
...
export const getAllUsers = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const users = await User.find(query).sort(sortOptions).limit(limit).skip(limit * (page - 1));
        res.status(200).json({
            success: true,
            users,
            totalUsers
...
export const getAllTeachers = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const teachers = await User.find(query).sort(sortOptions).limit(limit).skip(limit * (page - 1));
        res.status(200).json({
            success: true,
            teachers,
            totalTeachers
...
export const getAllAdmins = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const admins = await User.find(query).sort(sortOptions).limit(limit).skip(limit * (page - 1));
