...
export const getAllCourses = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const course = await Course.find({ title: { $regex: keyword, $options: 'i' }, category: { $regex: category, $options: 'i' } }).select("-lectures");
        res.status(200).json({
            success: true,
            course
        });
...
export const addLecture = CatchAsyncError(async (req, res, next) => {
    ...
    const fileUri = getDataUri(file);
    const myCloud = await cloudinary.v2.uploader.upload(fileUri.content, {
        folder: 'vinidraexam/lecture', resource_type: "video", timeout: 600000
    }, (err, result) => {
        if (err) {
...
export const addToPlaylist = CatchAsyncError(async (req, res, next) => {
    ...
    const isCourseAdded = user.playlist.find(item => item.course.toString() === req.params.id);
    if (isCourseAdded) {
...
export const removeFromPlaylist = CatchAsyncError(async (req, res, next) => {
    ...
    user.playlist = user.playlist.filter(item => item.course.toString() !== req.params.id);
    await user.save();
    res.status(200).json({
        success: true,
