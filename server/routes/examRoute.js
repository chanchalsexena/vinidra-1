...
import { createExam, getAllExams, getSingleExam, updateExam, deleteExam, createRazorpayOrder, verifyPayment, startExamAttempt, getAllExamAttempts, submitExamAttempt, enrollForExam, getTop10Scores, getResult, generateExamReport,  } from "../controllers/examController.js";
...
router.route("/exam").post(isAuthenticated, authorizeRole('admin', "teacher"), createExam);
router.route("/exam/:id").put(isAuthenticated, authorizeRole('admin', "teacher"), updateExam);
router.route("/exam/:id").delete(isAuthenticated, authorizeRole('admin', "teacher"), deleteExam);
router.route("/exam/report/:id").get(isAuthenticated, authorizeRole('admin', "teacher"), generateExamReport);
